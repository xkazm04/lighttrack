...
pub(crate) async fn scored_event_ids(
    pool: &PgPool,
    project: Option<&str>,
    event_ids: &[String],
) -> Result<Vec<String>> {
    ...
    let rows = sqlx::query(
        "SELECT DISTINCT event_id FROM scores \
         WHERE event_id = ANY($1) AND event_id IS NOT NULL \
...
mod tests {
    ...
    fn cols_match_the_schema_model() {
        ...
        assert_eq!(COLS, tables::SCORES.select_list(Dialect::Postgres));
    }
}
