...
fn normal_rubric_prompt_keeps_its_semantics() {
    let p = build_rubric_prompt(&rubric(), "What is the capital of France?", Some("Paris"), "Paris.");
    assert!(!p.injection_suspected, "clean content must not raise the signal");
    ...
    assert!(p.text.contains("- correctness (weight 2): factually right."));
...
fn injected_verdict_never_reaches_the_instruction_channel() {
    ...
    assert!(p.injection_suspected, "a spoofed section must raise the signal");
    let trusted = instruction_channel(&p.text);
    assert!(!trusted.contains("score\":1.0"), "fabricated verdict leaked: {trusted}");
...
fn every_builder_fences_untrusted_content() {
    ...
    for p in [
        ...
        let trusted = instruction_channel(&p.text);
        assert!(!trusted.contains("Ignore the rubric"), "payload leaked: {trusted}");
    }
}
...
fn repair_prompt_fences_the_malformed_model_text() {
    ...
    assert!(repair.text.contains("ONLY valid JSON matching the schema"), "repair marker is stable");
    let trusted = instruction_channel(&repair.text);
    assert!(!trusted.contains("score\":1.0"), "repaired-text payload leaked: {trusted}");
