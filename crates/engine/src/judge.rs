...
pub fn run_rubric_judge(
    ...
    judge_with(&gen, rubric, &prompt, model, samples, jobs)
}
...
/// math without live calls. Samples are generated by index — up to `jobs` concurrently — and
/// aggregated deterministically in index order, so `jobs == 1` and `jobs == k` are byte-identical.
fn judge_with(
    gen: &impl Generator,
    rubric: &Rubric,
    prompt: &str,
