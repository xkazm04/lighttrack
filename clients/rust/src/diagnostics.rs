...
pub(crate) fn no_project_message(base_url: &str) -> String {
    format!(
        "events are being dropped: no project is configured, and without an API key the server \
         ...
         LIGHTTRACK_PROJECT=<your-project-id> (or pass it to Client::new), or set LIGHTTRACK_KEY to \
         a project API key, which pins the project server-side. Target: {base_url}"
    )
}
